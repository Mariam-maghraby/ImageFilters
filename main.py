...
class ApplicationWindow(QtWidgets.QMainWindow):
    def __init__(self):
        ...
        self.ui.verticalLayout_3.addWidget(self.Canvas_for_Frequency_Domain_Image)
        ...
        self.ui.verticalLayout_5.addWidget(self.Canvas_for_Filtered_Image_Image)
        self.ui.verticalLayout_6.addWidget(self.Canvas_for_Frequency_Domain_filteredImage)
    ...
    def open_file(self):
        ...
        self.fileName, _ = QFileDialog.getOpenFileName(None, "QFileDialog.getOpenFileName()", "", "All Files (*);;csv Files (*.csv)", options=options)
        if self.fileName:
            ...
            self.ui.Frequency_Domain_Title.setText(self.translate("MainWindow", "<html><head/><body><p align=\"center\">Frequency Domain</p></body></html>"))
    ...
    def Mean_Filter(self):
        ...
        if len(original_img.shape) == 3:
           ...
           filtered_img = cv2.blur(converted_img, (self.fig_size, self.fig_size))
           ...
           self.draw_freq_filtered_Img()
            ...
            filtered_img = cv2.blur(original_img, (self.fig_size, self.fig_size))
    ...
    def Gaussian_Filter(self):
        ...
        if len(original_img.shape) == 3:
            converted_img = cv2.cvtColor(original_img, cv2.COLOR_BGR2HSV)
            filtered_img = cv2.GaussianBlur(converted_img, (self.fig_size, self.fig_size), 0)
            result_img = cv2.cvtColor(filtered_img, cv2.COLOR_HSV2RGB)
            self.Canvas_for_Filtered_Image_Image.axes.imshow(result_img,extent=[
            0.5, (result_img.shape[1]), 0.5, (result_img.shape[0]+0.5)])
            self.Canvas_for_Filtered_Image_Image.draw()
            ...
            filtered_img = cv2.GaussianBlur(original_img, (self.fig_size, self.fig_size), 0)
            self.Canvas_for_Filtered_Image_Image.axes.imshow(filtered_img, cmap='gray',extent=[
            0.5, (filtered_img.shape[1]), 0.5, (filtered_img.shape[0]+0.5)])
            self.Canvas_for_Filtered_Image_Image.draw()
            cv2.imwrite(self.saved_img, filtered_img)
    ...
    def Box_Filter(self):
         ...
         filtered_spatial_image=cv2.boxFilter(original_img, -1,(10, 10), normalize=True) 
    ...
    def Frequency_Filter(self):
         ...
         idft_shift = np.fft.ifftshift(filtered_dft_shift)  #Move the frequency domain from the middle to the upper left corner
         filtered_spatial_image = np.fft.ifft2(idft_shift)  # Fourier library function call
    ...
    def highPassFiltering(self,img,size):         #Transfer parameters are Fourier transform spectrogram and filter size
        h, w = img.shape[0:2]                     #Getting image properties
        h_center,w_center = int(h/2), int(w/2)                #Find the center point of the Fourier spectrum
        img[h_center-int(size/2):h_center+int(size/2), w_center-int(size/2):w_center+int(size/2)] = 0 #Center point plus or minus half of the filter size, forming a filter size that defines the size, then set to 0
        return img

    def lowPassFiltering(self,img,size):              #Transfer parameters are Fourier transform spectrogram and filter size
        h, w = img.shape[0:2]                          #Getting image properties
        h_center,w_center = int(h/2), int(w/2)                       #Find the center point of the Fourier spectrum
        img2 = np.zeros((h, w), np.uint8)                 #Define a blank black image with the same size as the Fourier Transform Transfer
        img2[h_center-int(size/2):h_center+int(size/2), w_center-int(size/2):w_center+int(size/2)] = 1    #Center point plus or minus half of the filter size, forming a filter size that defines the size, then set to 1, preserving the low frequency part
    ...
    def plot_original_histogram(self):
        ...
        print(hist.shape)   
        self.ui.Frequency_Domain_Title.setText(self.translate("MainWindow", "<html><head/><body><p align=\"center\">Original Histogram</p></body></html>"))
    ...
    def show_histogram(self):
        ...
        self.ui.Filtered_Image_Title.setText(self.translate("MainWindow", "<html><head/><body><p align=\"center\">Equalized Image</p></body></html>"))
        self.ui.Frequency_Domain_filtered_Title.setText(self.translate("MainWindow", "<html><head/><body><p align=\"center\">Equalized Histogram</p></body></html>"))
        self.ui.Frequency_Domain_Title.setText(self.translate("MainWindow", "<html><head/><body><p align=\"center\">Original Histogram</p></body></html>"))
        self.widgets_flag = 1
        self.equalized_img()
    ...
    def show_filters(self):
        ...
        self.ui.Frequency_Domain_filtered_Title.setVisible(True)
        self.ui.Frequency_Domain_Title.setText(self.translate("MainWindow", "<html><head/><body><p align=\"center\">Frequency Domain</p></body></html>"))
        self.ui.Filtered_Image_Title.setText(self.translate("MainWindow", "<html><head/><body><p align=\"center\">Filtered Image</p></body></html>"))
        self.ui.Frequency_Domain_filtered_Title.setText(self.translate("MainWindow", "<html><head/><body><p align=\"center\">Frequency Domain</p></body></html>"))
        self.choose_filter() 
        self.Canvas_for_Frequency_Domain_Image.setVisible(True)
